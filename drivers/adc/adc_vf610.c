...
static int vf610_adc_channel_setup(struct device *dev,
...
{
	const struct vf610_adc_config *config = dev->config_info;
...
static int vf610_adc_read(struct device *dev,
...
static void vf610_adc_start_channel(struct device *dev)
{
	const struct vf610_adc_config *config = dev->config_info;
	struct vf610_adc_data *data = dev->driver_data;
...
static int vf610_adc_init(struct device *dev)
{
	const struct vf610_adc_config *config = dev->config_info;
	struct vf610_adc_data *data = dev->driver_data;
	ADC_Type *base = config->base;
...
#if CONFIG_ADC_VF610_CLK_IPG
	adc_config.clockSource = adcIpgClock;
#elif CONFIG_ADC_VF610_CLK_IPG_DIV_2
	adc_config.clockSource = adcIpgClockDivide2;
#else /* CONFIG_ADC_VF610_CLK_ADACK */
	adc_config.clockSource = adcAsynClock;
#endif

#if CONFIG_ADC_VF610_CLK_DIV_RATIO_1
	adc_config.divideRatio = adcInputClockDiv1;
#elif CONFIG_ADC_VF610_CLK_DIV_RATIO_2
	adc_config.divideRatio = adcInputClockDiv2;
#elif CONFIG_ADC_VF610_CLK_DIV_RATIO_4
	adc_config.divideRatio = adcInputClockDiv4;
#else /* CONFIG_ADC_VF610_ADC_CLK_DIV_RATIO_8 */
	adc_config.divideRatio = adcInputClockDiv8;
#endif
...
#define VF610_ADC_INIT(n)						\
	static void vf610_adc_config_func_##n(struct device *dev);	\
									\
	...
	static struct vf610_adc_data vf610_adc_data_##n = {		\
		...
		ADC_CONTEXT_INIT_SYNC(vf610_adc_data_##n, ctx),	\
									...
									\
	DEVICE_AND_API_INIT(vf610_adc_##n, DT_INST_LABEL(n),		\
			    &vf610_adc_init, &vf610_adc_data_##n,	\
